...
: _host(host), _port(port), _mount(mount), _user(user), _pass(pass) {}

void NtripClient::begin(Stream& output) {
  _output = &output;
  _enabled = true;
  _connected = connectToCaster();
  _lastGgaSentMs = 0; // allow immediate GGA send after connection
  _lastDataReceivedMs = millis(); // start stale detection from connection time
}
...
void NtripClient::loop() {
  ...
  if (_connected && _client.connected() &&
    ...
    Serial.println("[NTRIP] Stale connection detected, forcing reconnect");
    _client.stop();
    _connected = false;
  ...
  if (!_connected || !_client.connected()) {
    // Non-blocking reconnection with rate limiting
    uint32_t now = millis();
    if (now - _lastReconnectAttemptMs >= RECONNECT_DELAY_MS) {
      oledPrintln("[NTRIP] Lost. Reconnect...");
      _client.stop();
      _connected = connectToCaster();
      _lastGgaSentMs = 0; // allow immediate GGA after reconnection
      _lastDataReceivedMs = millis();  // Reset health check on reconnect
  ...
  if (avail > 0) {
    ...
    if (len > 0) {
      ...
      _lastDataReceivedMs = millis();  // Track successful data reception
    } else if (len < 0) {
      // Read error - force reconnection
      Serial.println("[NTRIP] Read error, forcing reconnect");
      _client.stop();
      _connected = false;
    }
  }
...
bool NtripClient::connectToCaster() {
  ...
  _client.setTimeout(3000);  // 3 second timeout for all socket operations
  ...
  while (!_client.available() && millis() - t0 < 2000) delay(10);
...
void NtripClient::setGgaMinPeriodMs(uint32_t ms) {
  if (ms < 1000) ms = 1000; // protezione minima 1 s
  _ggaMinPeriodMs = ms;
}
...
void NtripClient::sendGGALine(const char* gga, size_t len) {
  ...
  uint32_t now = millis();
  if (_lastGgaSentMs != 0 && (now - _lastGgaSentMs) < _ggaMinPeriodMs) return; // RATE-LIMIT

  // Invia riga e normalizza terminatore a CRLF (richiesto da molti caster)
